...
export default function EnviosPage() {
  ...
  async function fetchFila() {
    setLoading(true);
    addLog("Sincronizando régua de cobrança automática...", "info");
    ...
    if (data) {
      ...
      setFila(elegiveis);
      addLog(`${elegiveis.length} registros automáticos identificados.`, "success");
    }
    setLoading(false);
  }
  ...
  async function fetchCustomEnvios() {
    ...
    setLoading(true);
    addLog(`Buscando vencimentos em ${customDate} com script D${customGatilho >= 0 ? '+' : ''}${customGatilho}...`, "info");
    
    const { data } = await supabase
        .from('devedores_ativos')
        .select('*')
        .eq('data_vencimento', customDate)
    ...
    if (data) {
        ...
        setFila(customizados);
        addLog(`${customizados.length} registros customizados encontrados.`, "success");
    }
    setLoading(false);
  }
  ...
  const salvarMensagemManual = async (cpf: string) => {
    ...
    setIsSaving(cpf);
    const { error } = await supabase.from('devedores_ativos').update({ mensagem_personalizada: textoParaSalvar }).eq('cpf', cpf);
    if (!error) {
  ...
  const processarProximoDaFila = async () => {
    if (filaQueue.length === 0) {
      ...
      addLog("Fila finalizada.", "success");
      return;
    }
  ...
  const dispararMensagem = async (item: any, viaAutopiloto = false) => {
    ...
    if (result.ok) sucessoTotal = true;
      ...
      if (errorStr.includes('"exists":false') || errorStr.includes('not exists')) {
        addLog("Ajustando 9º dígito...", "error");
        let numAlt = result.numUsado.length === 13 ? result.numUsado.slice(0, 4) + result.numUsado.slice(5) : result.numUsado.slice(0, 4) + '9' + result.numUsado.slice(4);
    ...
    if (sucessoTotal) {
      await supabase.from('devedores_ativos').update({ ultimo_gatilho: gatilhoNome, mensagem_personalizada: null }).eq('cpf', item.cpf);
      setDraftMessages(prev => { const next = { ...prev }; delete next[item.cpf]; return next; });
      setFila(prev => prev.filter(f => f.cpf !== item.cpf));
      addLog(`Sucesso: ${item.nome}`, "success");
      if (!viaAutopiloto) setSendingId(null);
      return true;
    } else {
      addLog(`Falha: ${item.nome}`, "error");
      if (!viaAutopiloto) setSendingId(null);
      return false;
    }
  };
  ...
  const descartarMensagem = async (item: any) => {
    if (!confirm(`Descartar ${item.nome}?`)) return;
      ...
      <div className="bg-white border border-slate-100 p-6 rounded-[32px] mb-8 shadow-sm flex flex-col md:flex-row items-end gap-6 animate-in fade-in slide-in-from-top-4">
          ...
          <div className="flex-1 w-full text-left text-left">
              ...
              >
                  ...
                  <option value={1}>D+1 (1 Dia de Atraso)</option>
                  <option value={5}>D+5 (5 Dias de Atraso)</option>
                  <option value={10}>D+10 (10 Dias de Atraso)</option>
                  <option value={15}>D+15 (Aviso Judicial)</option>
              </select>
          </div>
          <button 
            ...
            fila.map((item) => (
              <div key={item.cpf} className={`bg-white rounded-[24px] border transition-all duration-300 overflow-hidden ${expandedCpf === item.cpf ? 'border-blue-500 shadow-2xl ring-1 ring-blue-500/10' : 'border-slate-100 shadow-sm'}`}>
                  ...
                  <div className="flex items-center gap-6">
                     <div className="text-right">
                        ...
                        <div className="text-xs font-bold text-blue-600">
                            {item._forceDiff !== undefined ? `D${item._forceDiff >= 0 ? '+' : ''}${item._forceDiff}` : `D${calcularDiferencaDias(item.data_vencimento) >= 0 ? '+' : ''}${calcularDiferencaDias(item.data_vencimento)}`}
                        </div>
                     </div>
                ...
                {expandedCpf === item.cpf && (
                  <div className="px-6 pb-6 pt-2 bg-slate-50/30 border-t border-slate-50 animate-in fade-in slide-in-from-top-2 text-left">
                    <div className="space-y-4">
                      <div className="relative text-left">
                        <textarea 
                          className="w-full bg-white border border-slate-200 rounded-2xl p-4 text-sm text-slate-700 font-medium h-28 shadow-inner outline-none focus:ring-1 focus:ring-blue-200 pr-12 text-left"
