...
export default function FolhaDesempenhoFuncionario() {
  ...
  // OPERAÇÕES ADMIN: Salvar ou Editar Meta Primária
  const handleCreateMeta = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isAdmin || !formMetaNome.trim() || !formMetaValor) return;
    setIsProcessing(true);
    const { error } = await supabase.from('desempenho_metas').insert([{
      funcionario_id: id,
  ...
  // OPERAÇÕES ADMIN: Ativar Modo de Edição de um Apontamento
  const handleStartEditApontamento = (ap: any) => {
    ...
    setIsMetaFormExpanded(true); // Abre as configurações para edição imediata
  };
  ...
  // OPERAÇÕES ADMIN: Criar Novo Lançamento de Feedback Tático
  const handleCreateFeedback = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isAdmin || !formFbDesc.trim()) return;
    setIsProcessing(true);
    const { error } = await supabase.from('desempenho_feedbacks').insert([{
      funcionario_id: id,
      tipo: formFbTipo,
  ...
  // Teto dinâmico baseado em inteligência de margem de segurança de 15% acima do limite máximo
  const yMax = Math.max(targetMetaVal, maxMedido) * 1.15 || 100;
  
  // Geração automatizada das 5 subdivisões do eixo Y de cima para baixo
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 pt-8 pb-20 text-left">
      ...
      {/* PAINEL EXCLUSIVO DO ADMINISTRADOR: ESTRUTURA EXPANSÍVEL ESTILO CMMS */}
      {isAdmin && (
        <section className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden mb-10 w-full text-left">
          <div 
          ...
          >
            <div className="flex items-center gap-3">
               ...
               <span className="font-black text-[10px] uppercase tracking-[0.2em] text-slate-500">
                 Configurações Técnico-Operacionais
               </span>
            </div>
          ...
          {isMetaFormExpanded && (
            <div className="p-8 border-t border-slate-50 space-y-8 animate-in slide-in-from-top-4 duration-300">
              ...
              {/* FORMULÁRIO 1: CRIAR META PRIMÁRIA */}
              <form onSubmit={handleCreateMeta} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end pb-6 border-b border-dashed border-slate-100">
                ...
                <button type="submit" disabled={isProcessing} className="w-full py-3 bg-slate-900 text-white font-black text-[9px] uppercase tracking-widest rounded-xl hover:bg-blue-600 transition-all cursor-pointer shadow-md">
                  + Criar Meta
                </button>
              </form>
              ...
              <form onSubmit={handleCreateApontamento} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end pb-6 border-b border-dashed border-slate-100">
                ...
                <div className="text-left">
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1">Valor Medido Mês</label>
                ...
                <div className="text-left">
                  ...
                  <input type="month" value={formApMesAno} onChange={e => setFormApMesAno(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold outline-none text-slate-700 font-mono" required />
                </div>
              ...
              <form onSubmit={handleCreateFeedback} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end text-left">
                <div className="text-left">
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1">Vetor do Feedback</label>
      ...
      <section className="bg-white p-10 rounded-[40px] border border-slate-100 shadow-2xl mb-10 text-left">
        <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-8 flex items-center gap-2 select-none">
          <Calendar size={14} className="text-blue-600" /> Desempenho Técnico
        </h3>
        ...
        {metas.length === 0 ? (
          ...
          <div className="space-y-8">
            ...
            <div className="flex gap-2 border-b border-slate-100 pb-4 overflow-x-auto select-none">
              {metas.map((m) => (
                <div key={m.id} className="flex items-center gap-1.5 shrink-0 bg-slate-50 rounded-xl p-1">
                  ...
                  {isAdmin && (
                    <button onClick={() => handleDeleteItem('desempenho_metas', m.id)} className="p-1.5 text-slate-300 hover:text-red-500 transition-colors mr-1">
                      <Trash2 size={11} />
                    </button>
                  )}
                </div>
            ...
            {metaSelecionada && (
              <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100/50 flex justify-between items-center text-xs select-none">
                ...
                <div className="text-right">
                  <span className="block text-[8px] font-black text-slate-400 uppercase tracking-widest">Alvo Estipulado</span>
            ...
            {apontamentosDaMetaAtiva.length === 0 ? (
              <div className="py-12 bg-slate-50/50 border border-dashed border-slate-200 rounded-3xl text-center text-slate-400 font-bold uppercase text-[9px] tracking-widest select-none">Nenhuma medição lançada para esta competência.</div>
            ) : (
              <div className="flex w-full gap-2 items-stretch pt-6 pb-8 bg-slate-50/30 p-6 rounded-[32px] border border-slate-100 overflow-x-auto">
                
                {/* EIXO Y (ORDENADAS): MARCADORES DE ESCALA NUMÉRICA */}
                <div className="flex flex-col justify-between text-right text-[9px] font-mono font-black text-slate-400 w-20 h-64 pr-3 border-r border-slate-200 select-none shrink-0">
                  {ticksY.map((t, idx) => (
                ...
                {/* ÁREA MESTRE DE PLOTAGEM VETORIAL DAS BARRAS */}
                <div className="flex-1 h-64 relative flex items-end gap-8 justify-start pl-6 min-w-[400px]">
                  ...
                  {/* LINHA DE META ALVO (GOAL LINE): MAPEAMENTO VISUAL DE LINHA DE CORTE */}
                  {metaSelecionada && (
                    <div 
                    ...
                    >
                      <span className="absolute right-4 top-[-16px] bg-red-50 border border-red-100 text-red-600 font-black text-[8px] uppercase tracking-widest px-1.5 py-0.5 rounded shadow-sm">
                        Limite Meta: {metaSelecionada.valor_meta}
                      </span>
                    </div>
                  )}
                      ...
                      <div key={ap.id} className="flex flex-col items-center justify-end h-full relative w-12 shrink-0 z-20 group">
                        ...
                        <div 
                          ...
                          className={`w-full rounded-t-lg transition-all duration-500 shadow-md ${
                            atingiuMeta ? 'bg-emerald-500 hover:bg-emerald-600 shadow-emerald-100' : 'bg-blue-500 hover:bg-blue-600 shadow-blue-100'
                          }`}
                        ></div>
                        
                        {/* EIXO X (COORDENADAS): COMPETÊNCIA CRONOLÓGICA FORA DA ESCALA VERTICAL */}
      ...
      {/* SEÇÃO NOVA ESPELHADA DO CMMS: HISTÓRICO TABULAR NUMÉRICO (LARGURA COMPLETA) */}
      <section className="bg-white rounded-[32px] shadow-sm border border-slate-100 overflow-hidden mb-12 text-left w-full">
        <div className="p-6 bg-slate-50/50 border-b border-slate-100">
          <span className="font-black text-[10px] uppercase tracking-[0.2em] text-slate-500">
            Histórico Numérico de Lançamentos
          </span>
        </div>

        {apontamentosDaMetaAtiva.length === 0 ? (
          <div className="py-16 text-center text-slate-300 font-bold uppercase text-[10px] tracking-widest">Nenhuma medição cadastrada em banco para esta competência.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/30 border-b border-slate-100 text-slate-400">
                  <th className="p-6 text-[11px] font-black uppercase tracking-wider">Mês / Competência</th>
                  <th className="p-6 text-[11px] font-black uppercase tracking-wider text-center">Valor Medido</th>
                  <th className="p-6 text-[11px] font-black uppercase tracking-wider text-center">Meta Estipulada</th>
                  <th className="p-6 text-[11px] font-black uppercase tracking-wider text-center">Aproveitamento</th>
                  {isAdmin && <th className="p-6 text-[11px] font-black uppercase tracking-wider text-center">Gestão</th>}
                </tr>
              </thead>
                    ...
                    <tr key={ap.id} className="hover:bg-slate-50/40 transition-colors">
                      ...
                      <td className="p-6 text-center">
                        <span className={`inline-block px-3 py-1 rounded-full text-[9px] font-black uppercase border tracking-wider ${
                          atingiu ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'
                        }`}>
                          {percentualIndex.toFixed(1)}% Cumprido
                        </span>
                      </td>
                      {isAdmin && (
                        <td className="p-6 text-center">
      ...
      {/* SEÇÃO MATRIZ DE FEEDBACKS: DUAS COLUNAS COM TRAVA DE HACHURA (ESTILO SÊNIOR CMMS) */}
      <section className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start w-full text-left">
        ...
        <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-2xl min-h-[300px] text-left">
          ...
          {feedbacksPositivos.length === 0 ? (
            <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest py-10 text-center">Nenhum feedback positivo lançado até o momento.</p>
          ) : (
            <div className="space-y-2.5 text-left">
              {feedbacksPositivos.map((fb) => (
                <div key={fb.id} className="p-4 bg-emerald-50/10 border border-emerald-100/40 rounded-2xl flex justify-between items-center text-xs font-bold text-slate-700 hover:bg-emerald-50/20 transition-colors text-left">
                  ...
                  {isAdmin && (
                    <button onClick={() => handleDeleteItem('desempenho_feedbacks', fb.id)} className="p-1 text-slate-300 hover:text-red-500 transition-colors ml-3 shrink-0 cursor-pointer">
                      <Trash2 size={12} />
                    </button>
                  )}
                </div>
        ...
        {/* COLUNA DIREITA: PONTOS DE ATENÇÃO (COM MAPEAMENTO HACHURADO) */}
        <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-2xl min-h-[300px] text-left">
          <h4 className="font-black text-xs text-red-500 uppercase tracking-wider mb-6 pb-2 border-b flex items-center gap-2 select-none">
            🔴 Pontos de Atenção
          </h4>
          {feedbacksAtencao.length === 0 ? (
            <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest py-10 text-center">Nenhum ponto de atenção lançado.</p>
          ) : (
            <div className="space-y-2.5 text-left">
              {feedbacksAtencao.map((fb) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0 ml-4">
                    ...
                    {isAdmin && (
                      <button onClick={() => handleDeleteItem('desempenho_feedbacks', fb.id)} className="p-1 text-slate-300 hover:text-red-500 transition-colors cursor-pointer">
                        <Trash2 size={12} />
                      </button>
                    )}
                  </div>
